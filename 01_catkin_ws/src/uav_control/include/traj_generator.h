...
class TRAJ_GENERATOR
{
    public:
        ...
        void printf_param();
        swarm_msgs::UAVControlCMD Circle_trajectory_generation(float time_from_start);
        void pub_ref_traj();
...
void TRAJ_GENERATOR::init(ros::NodeHandle &nh)
{
    ...
    //【发布】参考轨迹（RVIZ显示）
    ref_trajectory_pub = nh.advertise<nav_msgs::Path>(uav_name + "/reference_trajectory", 10);
}
...
swarm_msgs::UAVControlCMD TRAJ_GENERATOR::Circle_trajectory_generation(float time_from_start)
{
    uav_cmd.header.stamp = ros::Time::now();
    uav_cmd.cmd = swarm_msgs::UAVControlCMD::CTRL_Traj;
