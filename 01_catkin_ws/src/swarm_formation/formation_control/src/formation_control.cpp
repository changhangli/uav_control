...
void SunrayFormation::init(ros::NodeHandle &nh_)
{
    ...
    nh.param<std::string>("file_path", file_path, "/home/uav/ros_ws/src/swarm_formation/formation_control/config/config.yaml");
    ...
    // 【订阅】
    formation_cmd_sub = nh.subscribe<swarm_msgs::Formation>("/swarm/formation_cmd", 10, &SunrayFormation::formation_cmd_callback, this);
    formation_cmd_ground_sub = nh.subscribe<swarm_msgs::Formation>("/swarm/formation_cmd/ground", 10, &SunrayFormation::formation_cmd_callback, this);
    orca_cmd_sub = nh.subscribe<swarm_msgs::OrcaCmd>(topic_prefix + "/orca_cmd", 10, &SunrayFormation::orca_cmd_callback, this);
    goal_sub = nh.subscribe<geometry_msgs::PoseStamped>("/goal_" + std::to_string(agent_id), 10, &SunrayFormation::goal_callback, this);
    // 【发布】
    uav_control_cmd = nh.advertise<swarm_msgs::UAVControlCMD>(topic_prefix + "/uav_control_cmd", 10);
    ugv_control_cmd = nh.advertise<swarm_msgs::UGVControlCMD>(topic_prefix + "/ugv_control_cmd", 10);
    orca_setup_pub = nh.advertise<swarm_msgs::OrcaSetup>(topic_prefix + "/orca/setup", 10);
    uav_setup_pub = nh.advertise<swarm_msgs::UAVSetup>(topic_prefix + "/setup", 10);
    // 【定时器】
    ...
    home_set = false;
    state = swarm_msgs::Formation::INIT;
    formation_type = 0;
    ...
    if (leader_id > 100)
    {
        leader_pose_sub = nh.subscribe<swarm_msgs::UGVState>("/ugv" + std::to_string(leader_id - 100) + "/swarm_ugv/ugv_state", 10, &SunrayFormation::ugv_leader_callback, this);
    }
    else if (leader_id <= 100)
    {
        leader_pose_sub = nh.subscribe<swarm_msgs::UAVState>("/uav" + std::to_string(leader_id) + "/swarm/uav_state", 10, &SunrayFormation::uav_leader_callback, this);
    }
    ...
    for (int i = 0; i < agent_num; i++)
    {
        ...
        if (agent_type == 0)
        {
            agent_state_sub[i] = nh.subscribe<swarm_msgs::UAVState>(topic_prefix + "/swarm/uav_state", 10,
                                                                     boost::bind(&SunrayFormation::uav_state_cb, this, _1, i));
        }
        // 【订阅】ugv状态数据
        else if (agent_type == 1)
        {
            agent_state_sub[i] = nh.subscribe<swarm_msgs::UGVState>(topic_prefix + "/swarm_ugv/ugv_state", 10,
...
void SunrayFormation::formation_cmd_callback(const swarm_msgs::Formation::ConstPtr &msg)
{
    // 当指令为FORMATION时，根据formation_type进行队形切换
    if (msg->cmd == swarm_msgs::Formation::FORMATION)
    {
        state = swarm_msgs::Formation::FORMATION;
        formation_name = msg->name;
        // 固定队形
        if (msg->formation_type == swarm_msgs::Formation::STATIC)
        {
            formation_type = swarm_msgs::Formation::STATIC;
            static_formation_pub(msg->name);
        }
        // 动态队形
        else if (msg->formation_type == swarm_msgs::Formation::DYNAMIC)
        {
            formation_type = swarm_msgs::Formation::DYNAMIC;
            if (dynamic_type == 1)
            {
                now_idx = 0;
            }
        ...
        else if (msg->formation_type == 3)
        {
            ...
            formation_type = swarm_msgs::Formation::LEADER;
            now_idx = 0;
        }
    }
    else if (msg->cmd == swarm_msgs::Formation::TAKEOFF)
    {
        state = swarm_msgs::Formation::TAKEOFF;
        mode_takeoff_time = ros::Time::now();
    }
    else if (msg->cmd == swarm_msgs::Formation::LAND)
    {
        state = swarm_msgs::Formation::LAND;
    }
    else if (msg->cmd == swarm_msgs::Formation::HOVER)
    {
        state = swarm_msgs::Formation::HOVER;
    }
    else if (msg->cmd == swarm_msgs::Formation::SET_HOME)
    {
        this->set_home();
    }
    else if (msg->cmd == swarm_msgs::Formation::RETURN_HOME)
    {
        if (!home_set)
        {
        ...
        state = swarm_msgs::Formation::FORMATION;
        formation_type = swarm_msgs::Formation::GOAL;
        orca_setup.header.stamp = ros::Time::now();
        orca_setup.cmd = swarm_msgs::OrcaSetup::GOAL_RUN;
...
// orca状态及控制指令回调
void SunrayFormation::orca_cmd_callback(const swarm_msgs::OrcaCmd::ConstPtr &msg)
{
    orca_cmd = *msg;
    orca_cmd_time = ros::Time::now();
    if (state != swarm_msgs::Formation::FORMATION || swarm_msgs::Formation::GOAL)
    {
        return;
    }
    // 如果是停止状态，则发送悬停指令
    if (msg->state == swarm_msgs::OrcaCmd::STOP)
    {
        uav_cmd.header.stamp = ros::Time::now();
        uav_cmd.cmd = swarm_msgs::UAVControlCMD::Hover;

        ugv_cmd.header.stamp = ros::Time::now();
        ugv_cmd.cmd = swarm_msgs::UGVControlCMD::HOLD;
    }
    // 如果是运行状态，则发送速度指令
    else if (msg->state == swarm_msgs::OrcaCmd::RUN)
    {
        ...
        ugv_cmd.header.stamp = ros::Time::now();
        // ugv_cmd.cmd = swarm_msgs::UGVControlCMD::POS_CONTROL_ENU;
        ugv_cmd.cmd = swarm_msgs::UGVControlCMD::VEL_CONTROL_ENU;
        // ugv_cmd.cmd = swarm_msgs::UGVControlCMD::POS_VEL_CONTROL_ENU;
    ...
    // 如果是到达状态，则发送位置指令
    else if (msg->state == swarm_msgs::OrcaCmd::ARRIVED)
    {
...
// 无人机状态回调
void SunrayFormation::uav_state_cb(const swarm_msgs::UAVState::ConstPtr &msg, int i)
{
...
// 无人车状态回调
void SunrayFormation::ugv_state_cb(const swarm_msgs::UGVState::ConstPtr &msg, int i)
{
    agent_state[i].header.stamp = msg->header.stamp;
    agent_state[i].pose.pose.position.x = msg->position[0];
    agent_state[i].pose.pose.position.y = msg->position[1];
...
void SunrayFormation::uav_leader_callback(const swarm_msgs::UAVState::ConstPtr &msg)
{
...
void SunrayFormation::ugv_leader_callback(const swarm_msgs::UGVState::ConstPtr &msg)
{
    leader_pose_time = msg->header.stamp;
    leader_pose[0] = msg->position[0];
    leader_pose[1] = msg->position[1];
...
{
    // 如果当前模式是动态队形
    if (state == swarm_msgs::Formation::FORMATION && formation_type == swarm_msgs::Formation::DYNAMIC)
    {
        if (dynamic_type == 0)
        {
    ...
    // 如果是领队模式
    else if (state == swarm_msgs::Formation::FORMATION && formation_type == swarm_msgs::Formation::LEADER)
    {
...
void SunrayFormation::leader_formation_pub()
{
    ...
    if ((agent_type == 0 && agent_id == leader_id) || (agent_type == 1 && agent_id == leader_id - 100))
    {
        ...
        if (orca_cmd.goal_pos[0] == goal_x && orca_cmd.goal_pos[1] == goal_y && orca_cmd.state == swarm_msgs::OrcaCmd::ARRIVED)
        {
            now_idx++;
        }
    }
...
{
    this->debug();
    if (state == swarm_msgs::Formation::FORMATION)
    {
    ...
    if (state == swarm_msgs::Formation::TAKEOFF && agent_type == 0)
    {
        float time_diff = ros::Time::now().toSec() - mode_takeoff_time.toSec();
        if (time_diff < 5 &&
            uav_state.control_mode != swarm_msgs::UAVSetup::CMD_CONTROL)
        {
        ...
        else if (time_diff > 10 && time_diff < 15)
        {
            ...
            uav_control_cmd.publish(uav_cmd);
                 ...
                 uav_state.control_mode == swarm_msgs::UAVSetup::CMD_CONTROL && uav_state.armed)
        {
            set_home();
            orca_setup.header.stamp = ros::Time::now();
            orca_setup.cmd = swarm_msgs::OrcaSetup::GOAL;
    ...
    else if (state == swarm_msgs::Formation::LAND && agent_type == 0)
    {
        uav_setup.header.stamp = ros::Time::now();
        uav_setup.cmd = swarm_msgs::UAVSetup::SET_CONTROL_MODE;
    ...
    else if (state == swarm_msgs::Formation::HOVER)
    {
