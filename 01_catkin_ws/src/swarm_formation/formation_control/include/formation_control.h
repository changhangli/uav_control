...
private:
    ...
    swarm_msgs::OrcaSetup orca_setup;                                       // 设置orca模式 目标点消息
    swarm_msgs::UAVControlCMD uav_cmd;                                      // 无人机控制指令
    swarm_msgs::UGVControlCMD ugv_cmd;                                      // 无人车控制指令
    swarm_msgs::UAVState uav_state;                                         // 无人机状态
    swarm_msgs::UAVSetup uav_setup;                                         // 无人机模式
    swarm_msgs::OrcaCmd orca_cmd;                                           // orca返回值
    ...
    void formation_cmd_callback(const swarm_msgs::Formation::ConstPtr &msg); // 阵型指令回调
    void orca_cmd_callback(const swarm_msgs::OrcaCmd::ConstPtr &msg);        // orca返回值回调
    void uav_state_cb(const swarm_msgs::UAVState::ConstPtr &msg, int i);     // 无人机状态回调
    void ugv_state_cb(const swarm_msgs::UGVState::ConstPtr &msg, int i);     // 无人车状态回调
    void ugv_leader_callback(const swarm_msgs::UGVState::ConstPtr &msg);     // 无人车领导者状态回调
    void uav_leader_callback(const swarm_msgs::UAVState::ConstPtr &msg);     // 无人机领导者状态回调
