...
void ORCA::init(ros::NodeHandle &nh)
{
	...
	cmd_pub = nh.advertise<swarm_msgs::OrcaCmd>(topic_prefix + "/orca_cmd", 10);
	...
	for (int i = 0; i < agent_num; i++)
	{
		...
		// 【订阅】订阅控制指令
		cmd_sub[i] = nh.subscribe<swarm_msgs::OrcaSetup>(topic_prefix + "/orca/setup", 1,
														  boost::bind(&ORCA::setup_cb, this, _1, i));
	}

	// 打印定时器
	check_timer = nh.createTimer(ros::Duration(1.0), &ORCA::checkAgentState, this);
	orca_state = swarm_msgs::OrcaCmd::INIT;
...
bool ORCA::orca_run()
{
	...
	int idx = agent_id - 1;
	if (!start_flag || orca_state == swarm_msgs::OrcaCmd::INIT || !agent_state_ready)
	{
		orca_state = swarm_msgs::OrcaCmd::INIT;
		OrcaCmd.state = orca_state;
	...
	if (orca_state == swarm_msgs::OrcaCmd::ARRIVED || orca_state == swarm_msgs::OrcaCmd::STOP)
	{
		OrcaCmd.linear[0] = 0.0;
		OrcaCmd.linear[1] = 0.0;
		OrcaCmd.angular[2] = 0.0;
	...
	if (arrived_goal)
	{
		...
		orca_state = swarm_msgs::OrcaCmd::ARRIVED;
		OrcaCmd.state = orca_state;
		OrcaCmd.linear[0] = 0.0;
		OrcaCmd.linear[1] = 0.0;
		OrcaCmd.angular[2] = 0.0;
	...
	else
	{
		...
		orca_state = swarm_msgs::OrcaCmd::RUN;
		OrcaCmd.state = orca_state;
...
void ORCA::setup_cb(const swarm_msgs::OrcaSetup::ConstPtr &msg, int i)
{
	if (msg->cmd == swarm_msgs::OrcaSetup::GOAL)
	{
	...
	else if (msg->cmd == swarm_msgs::OrcaSetup::GOAL_RUN)
	{
		sim->setAgentGoal(i, RVO::Vector2(msg->desired_pos[0], msg->desired_pos[1]));
	...
	else if (msg->cmd == swarm_msgs::OrcaSetup::STOP)
	{
		orca_state = swarm_msgs::OrcaCmd::STOP;
	}
	else if (msg->cmd == swarm_msgs::OrcaSetup::RUN)
	{
		orca_state = swarm_msgs::OrcaCmd::RUN;
		start_flag = true;
	}
