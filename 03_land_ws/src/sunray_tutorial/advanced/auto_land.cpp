...
int main(int argc, char **argv)
{
    ...
    // 【订阅】无人机状态
    ros::Subscriber uav_state_sub = nh.subscribe<swarm_msgs::UAVState>(uav_name + "/swarm/uav_state", 10, uav_state_callback);
    // 【订阅】目标点位置
    ros::Subscriber target_pos_sub = nh.subscribe<detection_msgs::TargetsInFrameMsg>(uav_name + "/sunray_detect/qrcode_detection_ros", 1, tagCallback);
    // 【订阅】任务结束
    ...
    while (ros::ok())
    {
        ...
        if (landing_point_num > 10 && (((abs(x_rel) < error_xy && abs(y_rel) < error_xy && abs(z_rel) < error_z)) || abs(z_rel) < 0.20))
        {
            ...
            // 禁止定时下压后直接 EMERGENCY_KILL（可能仍在空中切桨）
            // 改发正常 Land，由控制栈/飞控完成落地停桨
