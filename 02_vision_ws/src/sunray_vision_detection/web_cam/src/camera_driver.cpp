...
bool AbstractV4LUSBCam::start()
{
    ...
    if(web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_QUERYCAP), &cap) < 0)
    {
        if(errno == EINVAL)
    ...
    if(web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_CROPCAP), &cropcap) == 0)
    {
        crop.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        crop.c = cropcap.defrect; /* reset to default */
        if(web_cam::util::xioctl(file_dev, VIDIOC_S_CROP, &crop) < 0)
        {
            if(errno == EINVAL)
    ...
    if(web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_S_FMT), &fmt) < 0)
    {
    ...
    if(web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_G_PARM), &stream_params) < 0)
    {
    ...
    if (web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_S_PARM), &stream_params) < 0)
...
bool AbstractV4LUSBCam::start_capture()
{
    ...
    case IO_METHOD_MMAP:
        for (i = 0; i < buffers_count; ++i)
        {
            ...
            buf.index = i;
            if(web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_QBUF), &buf) < 0)
            {
    ...
    if (web_cam::util::xioctl(file_dev, VIDIOC_STREAMON, &type) < 0)
    {
...
bool AbstractV4LUSBCam::suspend()
{
    ...
    case IO_METHOD_USERPTR:
        type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if (web_cam::util::xioctl(file_dev, VIDIOC_STREAMOFF, &type) < 0)
        {
...
camera_image_t *AbstractV4LUSBCam::read_frame()
{
    ...
    case IO_METHOD_MMAP:
        ...
        buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        buf.memory = V4L2_MEMORY_MMAP;
        if (web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_DQBUF), &buf) < 0)
        {
            if(errno == EAGAIN)
                return nullptr;
            else if(errno == EIO){}
...
void AbstractV4LUSBCam::run_grabber(unsigned int &buffer_size)
{
    if(io_method == IO_METHOD_MMAP)
    {
        ...
        if(web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_REQBUFS), &req) < 0)
        {
            if(errno == EINVAL)
        ...
        for (buffers_count = 0; buffers_count < req.count; ++buffers_count)
        {
            ...
            if (web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_QUERYBUF), &buf) < 0)
            {
...
std::vector<capture_format_t> &AbstractV4LUSBCam::get_supported_formats()
{
    ...
    for(current_format.index = 0;
         web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_ENUM_FMT), &current_format) == 0;
...
bool AbstractV4LUSBCam::process_image(const void *src, int len, camera_image_t *dest)
{
    ...
    {
        if(monochrome) // actually format V4L2_PIX_FMT_Y16, but web_cam::utils::xioctl gets unhappy if you don't use the advertised type (yuyv)
...
void AbstractV4LUSBCam::adjust_camera()
{
    ...
    if(autofocus)
    {
        ...
        if (web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_QUERYCTRL), &queryctrl) < 0)
        {
            if (errno != EINVAL)
        ...
        else
        {
            ...
            if (web_cam::util::xioctl(file_dev, static_cast<int>(VIDIOC_S_CTRL), &control) < 0)
